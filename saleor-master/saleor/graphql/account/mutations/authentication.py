...
class SocialCreateToken(BaseMutation):
    """Mutation that authenticates a user and returns token and user data."""

    class Arguments:
        email = graphene.String(required=True, description="Email of a user.")
        socialId = graphene.String(required=True, description="Social Id of a user.")
    ...
    @classmethod
    def _retrieve_user_from_credentials(cls, email, socialId) -> Optional[models.User]:
        if email and socialId:
            user = models.User.objects.filter(
                email=email, is_active=True, socialId=socialId).first()
            if user:
                return user
        return None
    ...
    @classmethod
    def get_user(cls, _info, data):
        user = cls._retrieve_user_from_credentials(data["email"], data["socialId"])
        if not user:
            raise ValidationError(
                {
                    "email": ValidationError(
